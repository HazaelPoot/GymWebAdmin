...
namespace GymApi.Application.Services
{
    ...
    {
        private readonly IGenericSqlRepository<Suscripcion> _repository;
        private readonly IServicioService _servicioService;
        private readonly IMiembroService _miembroService;
        private readonly ICorreoService _correoService;
        private readonly IGymService _gymService;

        public SuscripcionService(IGenericSqlRepository<Suscripcion> repository, IGymService gymService, IMiembroService miembroService, IServicioService servicioService, ICorreoService correoService)
        {
        ...
        public async Task<Suscripcion> VerificarPago(Suscripcion entidad)
        {
            try
            {
                ...
                DateTime.TryParse(suscrip_editar.FechaPago, out DateTime fechaPago);
        ...
        public async Task<Suscripcion> Crear(Suscripcion entidad)
        {
            ...
            try
            {
                entidad.ClaveAcceso = Guid.NewGuid().ToString("N")[..11];
                ...
                await _correoService.SuscriptionEmail(susCreado);
        ...
        public async Task<Suscripcion> Validar(int idGym, string parametro)
        {
            ...
            try
            {
                ...
                DateTime.TryParse(result.FechaPago, out DateTime fechaPago);
        ...
        public async Task<bool> EliminarTodo(int idGimnasio)
        {
            try
            {
                List<Suscripcion> suscripcions = await Lista(idGimnasio);
                bool response = await _repository.EliminateRange<Suscripcion>(s => s.IdServicioNavigation.IdGimnasio == idGimnasio);
